...
Priority::Priority(std::vector<Process> procs) : processes(procs) {
...
void Priority::execute() {
    ...
    std::cout << "\n=== Priority Scheduling Execution ===" << std::endl;
    ...
    while (completed.size() < processes.size()) {
        ...
        for (int i = 0; i < processes.size(); i++) {
            if (!isCompleted[i] && processes[i].getArrivalTime() <= currentTime) {
                if (processes[i].getPriority() < highestPriority) {
                    ...
                    if (highestPriorityIdx == -1 || processes[i].getArrivalTime() < processes[highestPriorityIdx].getArrivalTime()) {
                        highestPriorityIdx = i;
                    }
                }
            }
        ...
        Process& p = processes[highestPriorityIdx];

        int waitTime = currentTime - p.getArrivalTime();
        p.setWaitingTime(waitTime);

        std::cout << "Time " << currentTime << ": P" << p.getPid() << " (Priority " << p.getPriority() << ") starts executing" << std::endl;
...
void Priority::displayResults() {
    gantt.display();
    
    std::cout << "\n=== Results ===" << std::endl;
    std::cout << "PID\tArrival\tBurst\tPriority\tCompletion\tWaiting\tTurnaround" << std::endl;
